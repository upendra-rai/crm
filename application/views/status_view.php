			...
			<div class="inner-wrapper">
			  <?php $this->load->view('common/sidemenu'); ?>
				...
				<section role="main" class="content-body" >
								 ...
								 <header class="panel-heading" style="padding:0px 15px;">
								     	...
								     	<div class="heading_box" style=" border-bottom: none; padding: 6px 2px; height:36px; background-color:#27293d;">
									        ...
									        <h2 class="panel-title" style="color:#dbd6d6; font-size:16px; line-height:10px; text-transform:uppercase;">
														...
														Status List <i class="ion-chevron-right" style="padding:0px 10px; color:#0088cc;"></i> <?php  if(isset($product_list[0]->product_sub_category_name)){ echo $product_list[0]->product_sub_category_name;  } ?>
														...
														<a href="<?php echo base_url(); ?>/status/add"type="btn" class="btn-transparent btn-green" style="position:absolute; right:12px; top:8px;">Add Source</a>
													</h2>
									    </div>
								</header>
	               ...
	               <?php $i = 1; if(isset($status_details)) {foreach($status_details as $row) {?>
                                                            <tr>
														  <td><?php echo $i++;  ?></td>
                          	                            <td><?php echo $row->name; ?></td>
														 <td>
                    <a 
                     href="<?php echo base_url(); ?>/status/edit_status/<?php echo $row->id; ?>"> <button type="button" class="btn-transparent "  style=" width:100%;"><i class="fa fa-pencil"></i>
                   </a>
                      </td>
    ...
    $(document).on('click','button[id=confirm_del_action]',function(){
		...
		$(document).on('click','button[id=del_bt]',function(){
			   var del_id = $(this).data("del_id");
							$.ajax({
		              url : '<?php echo base_url(); ?>/status/delete_status',
								  method: 'POST',
									data:{vv: del_id},
									success:function(data){
										if(data === 'success'){
